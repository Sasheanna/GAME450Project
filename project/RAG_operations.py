...
session_collection = chromadb.Collection
...
class OllamaEmbeddingFunction:
    ...
    def __call__(self, input: List[str]) -> List[List[float]]:
        ...
        for index in range(0, len(input)):
            ...
            test = ollama.embed(model=self.model_name, input=input[index]).embeddings
...
def load_files_on_start():
    
    # for accessing the global variable later here
    ...
    session_collection = setup_chroma_db(
        chunks, 
        ollama_model=embedding_model
    )

def query_collection(collection: chromadb.Collection, query):
